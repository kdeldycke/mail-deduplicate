...
- Fix the documentation's broken links: `pipx`'s installation guide moved, and the `ctime` note now cites Python's `os.path.getctime` docs. Link checks skip sites answering `403` to crawlers and GitHub's JavaScript-rendered anchors. Closes [#899](https://github.com/kdeldycke/mail-deduplicate/issues/899).
- Streamline the internals ahead of the release. The hashing and selection steps now share a single pool-startup, fan-out and cache-consultation implementation instead of maintaining one copy each. Actions dispatch through an explicit verb-to-operation table and know which subset of mails they target, replacing seven wrapper functions, `globals()` lookups and scattered string parsing of action IDs. Statistics derive their category from their name, and the `StrEnum` backport shim is defined once for the whole package. The CLI is unchanged; Python importers must switch to `Strategy.function`/`Strategy.apply()`, `Action.perform()` and `DuplicateSet.select()`, which replace `strategy_function`/`apply_strategy()`, `perform_action()` and `categorize_candidates()`.
- Document in `--jobs`' help that it parallelizes the selection step too, not just the hashing.
- Add a `hardlink-discarded` action, which replaces each discarded mail by a hardlink to the copy kept in its duplicate set instead of removing it. The mails all stay in their folders under their own names, so `maildir` flags remain per-folder, but the copies share one file and the space the others took is reclaimed. Only copies identical byte for byte are linked, unless `--hardlink-differing` is passed: linking a mail that differs swaps its content for the kept copy's. Mails of a file-based box, mails already sharing the file of their copy and mails on another filesystem are left alone and reported apart. Closes [#164](https://github.com/kdeldycke/mail-deduplicate/issues/164).
- Add a tutorial section on hardlinking, walking through two `maildir` accounts holding the same mails, and a design page section on how the links are made and what they share.
- Add `--cache` and `--cache-path` to reuse the hashes computed by previous runs, so unchanged mails are no longer read nor parsed again: a second run over a 20,000-mail maildir takes about half the time of an uncached one, and its hashing step alone is five times faster. Hashes are kept in a SQLite database under the platform's cache directory, an entry is only trusted while the size and modification time of the file backing its mail are unchanged, and the whole cache is discarded as soon as `--hash-header`, `--hash-body` or `--time-source` changes. Off by default. Closes [#87](https://github.com/kdeldycke/mail-deduplicate/issues/87).
- Speed up the hashing step by about `2x`: the table of canonical headers is no longer rendered for every mail only to be discarded above debug verbosity, and a mail's location is now read from its box's own index instead of costing an extra `open()` and `stat()` per mail. On a 20,000-mail maildir the whole run gets about `2.6x` faster with the default `--hash-body skip`, and `2.3x` with `raw` and `normalized`. Refs [#87](https://github.com/kdeldycke/mail-deduplicate/issues/87).
- Cut the memory retained per mail from about `950` to `600` bytes, by deriving a mail's path from its box on demand instead of storing it, grouping mails by hash in lists instead of sets, and sharing the empty parsing defects of dehydrated mails. Refs [#87](https://github.com/kdeldycke/mail-deduplicate/issues/87).
- Correct the per-mail memory figures in the documentation, which claimed a few hundred bytes.
- Hash mails in worker processes instead of worker threads, making `--jobs` pay off for the first time: the hashing step gets `2.6x` faster at `--jobs=8` with the default `--hash-body skip`, `3.0x` with `raw` and `4.0x` with `normalized`, against `0.88x` to `0.97x` before, where raising it was a net loss. Hashing a mail is Python-level work that the interpreter lock serializes, so threads could only ever contend over it. Only a hash and two numbers travel back from a worker, never the mail. Applies to folder-based boxes (`maildir`, `MH`, `eml`), whose mails each own a file; runs over `mbox`, `babyl` and `mmdf` stay sequential. Falls back to hashing in-process where workers cannot be started.
- Halve the file-system calls the hashing step makes over a `maildir`, from four per mail to two. Its iterator re-checks that every key still resolves to a file, which the run then stats again on its own account. Worth little on a local disk, measurably more where each call crosses a network.
- Stop comparing every pair of a duplicate set against the thresholds. The size check is settled from the extremes of the whole set, and mails carrying the same body are diffed once between them rather than once per pair, so a set of copies costs about what a pair does. A single set of 200 copies took `1.90s` and now takes `0.04s`; it used to cost more than 20,000 mails arranged in pairs, and grew with the square of the set.
- Cut what a run says by seven eighths, from 32,000 lines to 4,000 on a 20,000-mail corpus, taking output from a quarter of a cached run down to `8%`. Mails and duplicate sets are named at `--verbosity DEBUG` only, the thresholds in force are announced once instead of once per set, and a dry run reports through the same progress trail as a real one rather than warning about every mail it left alone. The selection step gained a progress bar, being the longest one left once hashes come from the cache.
...
- Apply the selection strategies across worker processes too, so `--jobs` now speeds up both expensive steps rather than only the hashing. Duplicate sets share nothing with one another, which makes a set the natural unit of work; sets holding a single mail are settled without leaving the main process. On a 20,000-mail maildir a first run drops from `6.0s` to `1.8s` at `--jobs=4`, and a cached second run, where the selection is nearly all that is left, from `1.7s` to `1.3s`. Workers report what they decided rather than shipping mails back, and the parent replays their messages under each set's heading so the log reads as it would from a sequential run.
- Stop styling the heading of every duplicate set before knowing whether it will be logged. Most sets hold a single mail and report at debug level, where the rendered result was thrown away.
- Cover the parallel hashing of a compiled binary in the CLI test suite, over a small mail box committed as a fixture. Worker processes start by re-launching the executable, which in a frozen binary reaches our own entry point instead of the worker bootstrap, and nothing else exercised the call that tells the two apart.
- Prune the hash cache as it is used, so it tracks your mails instead of growing forever: every run drops the entries of mails that vanished from the boxes it opened, and of boxes that are no longer on disk at all. Boxes not passed on the command line are left untouched.
- Keep a run alive when its hash cache cannot be written at the end, because another run holds it or the disk filled up. Runs sharing a database now wait up to 30 seconds for it, and losing that race only costs the next run its head start.
- Add `platformdirs` to the runtime dependencies, which resolves the per-platform location of the hash cache.
- Add a performance page to the documentation, covering where a run spends its time, the memory it holds, the cost of each hashing option, and how to enable and manage the hash cache.
- Guard the performance work with regression tests on the quantities that drive it: the bytes retained per mail, the number of times each mail is read while hashing, and what a dehydrated mail still carries.
