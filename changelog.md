...
- Add `--cache` and `--cache-path` to reuse the hashes computed by previous runs, so unchanged mails are no longer read nor parsed again: a second run over a 20,000-mail maildir drops from `4.3s` to `2.5s`. Hashes are kept in a SQLite database under the platform's cache directory, an entry is only trusted while the size and modification time of the file backing its mail are unchanged, and the whole cache is discarded as soon as `--hash-header`, `--hash-body` or `--time-source` changes. Off by default. Closes [#87](https://github.com/kdeldycke/mail-deduplicate/issues/87).
- Speed up the hashing step by about `2x`: the table of canonical headers is no longer rendered for every mail only to be discarded above debug verbosity, and a mail's location is now read from its box's own index instead of costing an extra `open()` and `stat()` per mail. On a 20,000-mail maildir the whole run goes from `11.3s` to `4.3s` with the default `--hash-body skip`, from `12.5s` to `5.4s` with `raw` and from `15.7s` to `6.9s` with `normalized`. Refs [#87](https://github.com/kdeldycke/mail-deduplicate/issues/87).
